...
"<root>
    ...
    foreach my $k ( sort keys %tmpH )
    {
        ...
        if ( $k =~ /bkpJob/ )
        {
			...
			elsif ( $k eq "clean" )          { $cleanOlderThan = $v ; &printLog(3,"CLEAN OLDER THAN :: $v\n") };
        } else {
...
{
	my $pref  = $_[0];
	my $folder = substr($pref, 0, rindex($pref, "/"));
	my $name   = substr($pref, rindex($pref, "/")+1);

	#my $files = &listFiles($pref, ".log.tgz");
	my $fls   = &listLog($folder, $name);
...
;
	...
	my $fls  = &listLog($outputFolder, $name);
...
sub listLog
{
	...
	#print " OPENING DIR: $folder for $name\n";
    if ($error)
    {
