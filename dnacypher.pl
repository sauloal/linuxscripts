...
if ( ! ( $ARGV[0] ) && ( $ARGV[1] ))
{
	die "USAGE: dnacypher [ENC/DEC] FILENAME\n";
}
elsif ( -f $ARGV[1] )
{
	open FILE, "<$ARGV[1]" or die "COULD NOT OPEN FILE $ARGV[1]: $!\n";
	my $text = join("",<FILE>);
	close FILE;

	if ($ARGV[0] eq "ENC")
	{
		print &dnaencoder($text);
	}
	elsif ($ARGV[0] eq "DEC")
	{
		print &dnadecoder($text);
	}
	else
	{
		die "USAGE: dnacypher [ENC/DEC] FILENAME\nYOU HAVE DEFINED $ARGV[0] AS A INVALID COMMAND\n";
	}
}
else
{
	die "FILENAME $ARGV[1] DOESNT EXISTS\n";
}
...
#         };
	...
	$_ =~ tr/ACGT/0123/;
        ...
        $_ =~ s/(.)(.)(.)(.)/chr(64*$1+16*$2+4*$3+$4)/gex;
                # sigamos os conselhos do sabio jack.. vamos por partes aqui tb
